...
def kaiming_uniform(fan_in, fan_out, shape=None, device=None, dtype="float32", nonlinearity="relu", **kwargs):
    assert nonlinearity == "relu", "Only relu supported currently"
    ### BEGIN YOUR SOLUTION
    bound = math.sqrt(2) * math.sqrt(3/fan_in)
    if shape is None:
        return rand(fan_in, fan_out, low=-bound, high=bound, device=device, dtype=dtype)
    else:
        return rand(*shape, low=-bound, high=bound, device=device, dtype=dtype)
    ### END YOUR SOLUTION
