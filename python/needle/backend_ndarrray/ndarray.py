#this file is equivalent to the backend_numpy.py. this is backend_ndarray
import operator
import math
...
class BackendDevice:
    """A backend device, wrapps the implementation module.
       This class is equivalent to the DeviceCPU class in backend_numpy.py"""
...
class NDArray:
    ...
    def fill(self, value):
        ...
        self._device.fill(self._handle, value)  #每一种device都具体实现了fill的方法
    ...
    def numpy(self):
        ...
        return self.device.to_numpy(
            self._handle, list(self.shape), list(self.strides), self._offset
        )
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape)!=prod(new_shape):
            raise ValueError('New shape does not match the original one.')
        if self.is_compact()==False:
            raise ValueError('NDArray is not compact.')
        
        product = prod(new_shape)
        new_strides = []
        for i in range(len(new_shape)):
            product = int(product/new_shape[i])
            new_strides.append(product)

        return NDArray.make(shape=new_shape, strides=tuple(new_strides), device=self._device, handle=self._handle, offset=self._offset)
    ...
    def permute(self, new_axes):
        ...
        return NDArray.make(shape=tuple(new_shape), strides=tuple(new_strides), device=self._device, handle=self._handle, offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(new_shape)):
            if new_shape[i]!=self._shape[i]:
                if self._shape[i]!=1:
                    raise ValueError("new shape is illegal")
                else:
                    broadcast_axes.append(i)
        new_strides = list(self._strides)
        for i in broadcast_axes:
            new_strides[i] = 0
        return NDArray.make(shape=new_shape, strides=tuple(new_strides), device=self._device, handle=self.compact()._handle, offset=self._offset)
    ...
    def flip(self, axes):
        ...
        flip_arr =  NDArray.make(shape=self._shape, strides=tuple(new_strides), device=self._device, handle=self._handle, offset=offset)
        return flip_arr.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        ### BEGIN YOUR SOLUTION
        nparray = self.numpy()
        nparray = np.pad(nparray, axes)
        return NDArray(nparray, self.device)
...
def max_uni(array, axis=None, keepdims=False):
