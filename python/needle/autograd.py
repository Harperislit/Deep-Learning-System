...
class Tensor(Value):
    ...
    def sum(self, axes=None):
        print("autograd sum is called. axes: ", axes)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if (type(partial_adjoints) is not tuple):
            partial_adjoints = tuple([partial_adjoints])
        for input_, partial_adjoint in zip(node.inputs, partial_adjoints):
            if input_ in node_to_output_grads_list.keys():
                node_to_output_grads_list[input_].append(partial_adjoint)
            else:
                node_to_output_grads_list[input_] = [partial_adjoint]
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    """Post-order DFS"""
    ### BEGIN YOUR SOLUTION
    for succe in node.inputs:
        if succe not in visited:
            visited.append(succe)
            topo_sort_dfs(succe, visited, topo_order)
