...
class ConvBN(ndl.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, device=None, dtype="float32"):
        ...
        self.batch_norm = nn.BatchNorm2d(dim=self.out_channels, device=device)
        self.relu = nn.ReLU()
...
class ResNet9(ndl.nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.convBN2(x1)
        x2 = self.convBN3(x1)
        x2 = self.convBN4(x2) + x1
        x2 = self.convBN5(x2) 
        x2 = self.convBN6(x2)
        x3 = self.convBN7(x2)
        x = self.convBN8(x3) + x2
        x = self.flatten(x)
        x = self.linear1(x)
        x = self.relu(x)
        x = self.linear2(x)
...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, h=None):
        ...
        seq_len, bs = x.shape
        x = self.embedding(x)
        output, h = self.sequence_model(x, h)
        output = ndl.ops.reshape(output, (seq_len*bs, self.hidden_size))
