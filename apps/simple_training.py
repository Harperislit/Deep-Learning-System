...
def epoch_general_ptb(data, model, seq_len=40, loss_fn=nn.SoftmaxLoss(), opt=None,
    ...
    num_batches = 0
    num_samples = 0
    for i in range(0, nbatch-seq_len, seq_len):
        num_batches+=1
        if opt is not None:
            opt.reset_grad()
        x, y = ndl.data.get_batch(data, i, seq_len, device, dtype)
        x = ndl.Tensor(x, device=device, dtype=dtype)
        output, h = model(x)
        pred = np.argmax(output.cached_data.numpy(), 1)
        num_samples += pred.shape[0]
        total_correct += np.sum(pred==y)
    ...
    loss = total_loss.numpy()/num_batches
    return acc, loss   
