...
#define BASE_THREAD_NUM_SQRT 16
...
CudaDims CudaTwoDim(size_t size1, size_t size2) {
  CudaDims dim;
  size_t num_blocks1 = (size1 + BASE_THREAD_NUM_SQRT - 1) / BASE_THREAD_NUM_SQRT;
  size_t num_blocks2 = (size2 + BASE_THREAD_NUM_SQRT - 1) / BASE_THREAD_NUM_SQRT;
  dim.block = dim3(BASE_THREAD_NUM_SQRT, BASE_THREAD_NUM_SQRT, 1);
  dim.grid = dim3(num_blocks1, num_blocks2, 1);
  return dim;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset, int funcType) {
  /**
   ...
   *   functype: 0-compact function, 1-setitem_ewise
   */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  {
      CudaVec compInd;
      compInd.size = shape.size;
      int ind = gid;
      for (int i=shape.size-1; i>=0; --i)
      {
          compInd.data[i] = ind%shape.data[i];
          ind = ind/shape.data[i];
      }
      
      int nonCompInd = offset;
      for (int i=0; i<strides.size; ++i)
      {
          nonCompInd += strides.data[i]*compInd.data[i];
      }
      if (funcType==0)
      {
          out[gid] = a[nonCompInd];
      }
      else
      {
          out[nonCompInd] = a[gid];
      }
  }
...
void EwiseSetitem(const CudaArray& a, CudaArray* out, std::vector<int32_t> shape,
  ...
  CompactKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, a.size, VecToCuda(shape),
                                         VecToCuda(strides), offset, 1);
  /// END YOUR SOLUTION
}


__global__ void ScalarSetItemKernel(const int val, scalar_t* out, size_t size, CudaVec shape,
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid<size)
  {
      ...
      {
          if (a[gid*reduce_size+i]>max) max=a[gid*reduce_size+i];
      }
      out[gid] = max;
  }
